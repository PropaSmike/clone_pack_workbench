...
class GuideWindow(tk.Toplevel):
    ...
    def __init__(self, master: tk.Misc, root: Path):
        ...
        width = min(980, self.winfo_screenwidth() - 80)
        height = min(720, self.winfo_screenheight() - 120)
        x = min(master.winfo_rootx() + 60, self.winfo_screenwidth() - width - 20)
        y = min(master.winfo_rooty() + 60, self.winfo_screenheight() - height - 60)
